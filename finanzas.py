...
class DatabaseManager:
    ...
    def _init_default_categories(self):
        ...
        try:
            ...
            st.warning(f"Error inicializando categorías: {e}")
    ...
    def marcar_ingreso_recibido(self, id: int, recibido: bool) -> bool:
        ...
        response = self.client.table('ingresos_mensuales').update({
            'recibido': 1 if recibido else 0, 'fecha_recibo_real': fecha_recibo
        }).eq('id', id).execute()
        return len(response.data) > 0
    ...
    def obtener_gastos_fijos_mensuales(self, mes: int, anio: int) -> List[Dict]:
        ...
        for r in response.data:
            ...
            result.append({
                'id': r['id'], 'gasto_fijo_id': r['gasto_fijo_id'], 'monto': r['monto'],
                'pagado': r['pagado'], 'fecha_pago_real': r['fecha_pago_real'],
                'notas': r['notas'], 'nombre': gf['nombre'],
                'fecha_pago': gf['fecha_pago'], 'categoria_nombre': cat['nombre'],
                'color': cat['color'], 'icono': cat['icono'], 'categoria_id': gf['categoria_id']
            })
        result.sort(key=lambda x: x['fecha_pago'] or 99)
        return result
    ...
    def marcar_gasto_fijo_pagado(self, id: int, pagado: bool) -> bool:
        ...
        response = self.client.table('gastos_fijos_mensuales').update({
            'pagado': 1 if pagado else 0, 'fecha_pago_real': fecha_pago
        }).eq('id', id).execute()
        return len(response.data) > 0
    ...
    def establecer_presupuesto(self, categoria_id: int, mes: int, anio: int, 
        ...
        try:
            ...
            st.error(f"Error estableciendo presupuesto: {e}")
            return False
        ...
        if prestamos:
            ...
            for prestamo in prestamos:
                with st.expander(f"💳 {prestamo['nombre']} - {formatear_moneda(prestamo['monto_total'])}"):
                    ...
                    col1, col2, col3 = st.columns(3)
                    col1.metric("Monto Original", formatear_moneda(prestamo['monto_total']))
                    col1.metric("Total Pagado", formatear_moneda(total_pagado))
                    col2.metric("Saldo Pendiente", formatear_moneda(saldo))
                    col2.metric("Cuota Mensual", formatear_moneda(prestamo['cuota_mensual']))
                    col3.metric("Tasa", f"{prestamo['tasa_interes']}%")
                    ...
                    with col1:
                        with st.form(f"pago_{prestamo['id']}"):
                            monto_pago = st.number_input("Monto (S/)", min_value=0.0, step=0.01, value=prestamo['cuota_mensual'])
    ...
    with tab2:
        ...
        with st.form("nuevo_prestamo"):
            ...
            with col1:
                ...
                monto_total = st.number_input("Monto total (S/)", min_value=0.0, step=0.01)
                tasa_interes = st.number_input("Tasa (%)", min_value=0.0, step=0.1)
            with col2:
                cuota_mensual = st.number_input("Cuota mensual (S/)", min_value=0.0, step=0.01)
...
def pagina_ahorros(db: DatabaseManager, mes: int, anio: int):
    ...
    with col2:
        ...
        with st.form("nuevo_ahorro"):
            ...
            monto = st.number_input("Monto (S/)", min_value=0.0, step=0.01)
            fecha = st.date_input("Fecha", value=datetime.now())
...
def pagina_metas(db: DatabaseManager, mes: int, anio: int):
        ...
        if metas:
            for meta in metas:
                with st.expander(f" {meta['nombre']} - {formatear_moneda(meta['monto_actual'])} / {formatear_moneda(meta['monto_objetivo'])}"):
                    ...
                    col2.metric("Falta", formatear_moneda(meta['monto_objetivo'] - meta['monto_actual']))
                    col3.metric("Progreso", f"{progreso:.1f}%")
                    st.progress(min(progreso / 100, 1.0))
                    
                    st.markdown("---")
                    with st.form(f"aporte_{meta['id']}"):
                        monto_aporte = st.number_input("Monto (S/)", min_value=0.0, step=0.01)
                        if st.form_submit_button("Aportar"):
                            if monto_aporte > 0:
                                db.agregar_aporte_meta(meta['id'], monto_aporte, datetime.now().isoformat())
    ...
    with tab2:
        ...
        with st.form("nueva_meta"):
            nombre = st.text_input("Nombre")
            monto_objetivo = st.number_input("Monto objetivo (S/)", min_value=0.0, step=0.01)
            ...
            if submit:
                if nombre and monto_objetivo > 0:
                    ...
                    db.agregar_meta(nombre, monto_objetivo, fecha_str, prioridad)
...
def pagina_presupuestos(db: DatabaseManager, mes: int, anio: int):
    ...
    if total_ingresos > 0:
        ...
        st.info(f"💡 **Sugerencia 50/30/20:** Necesidades: {formatear_moneda(total_ingresos*0.5)}, Deseos: {formatear_moneda(total_ingresos*0.3)}, Ahorro: {formatear_moneda(total_ingresos*0.2)}")
    ...
    with st.form("configurar_presupuesto"):
        ...
        with col1:
            ...
            if categoria_options:
                ...
                categoria_id = None
        with col2:
            monto_presupuesto = st.number_input("Monto (S/)", min_value=0.0, step=0.01)
...
def pagina_historial(db: DatabaseManager):
    ...
    while fecha_actual <= fecha_fin:
        ...
        datos.append({
            ...
            'Ingresos': ingresos, 'Gastos Fijos': gf,
            'Gastos Variables': gv, 'Ahorros': ahorros, 'Saldo': saldo
        })
    ...
    if datos:
        ...
        st.dataframe(df.style.format({'Ingresos': 'S/ {:,.2f}', 'Gastos Fijos': 'S/ {:,.2f}', 
                                       'Gastos Variables': 'S/ {:,.2f}', 'Ahorros': 'S/ {:,.2f}', 'Saldo': 'S/ {:,.2f}'}), use_container_width=True)
    else:
